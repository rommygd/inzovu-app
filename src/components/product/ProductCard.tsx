...
export default function ProductCard({ product }: { product: Product }) {
  ...
  const toggleLike = () => {
    ...
    if (isLiked) {
      updated = liked.filter((id: string) => id !== product.id);
    } else {
      updated = [...liked, product.id];
    }
  ...
  return (
    <div className="group relative bg-white dark:bg-gray-900 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 dark:border-gray-800 hover:border-primary/20">
      ...
      <div className="p-4 space-y-3">
        {/* Title */}
        <Link to={`/product/${product.id}`}>
          <h3 className="font-semibold text-sm line-clamp-2 leading-tight min-h-[2.5rem] group-hover:text-primary transition-colors duration-200">
            {product.name}
          </h3>
        </Link>
